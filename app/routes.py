#definisce tutte le pagine e le API dell'applicazione.
#ogni funzione @app.route corrisponde a un URL
#Quando l'utente visita quella URL, Flask esegue la funzione corrispondente
...
@login_required
def index():
    return render_template('index.html', title='About RaffCity')

#gestisce il login, se utente già loggato, manda alla home
#se no, verifica username e password nel db
#se corretti esegue il login.
...
def user(username):
    ...
    return render_template('user.html', user=user, title='RaffCity User')
...
#API JSON chiamata da JavaScript della mappa.
...
#API JSON chiamata da JS della mappa per restituire tutte le fermate salbvate dall'utente in formato JSON.
#dati utilizzarti da Leaflet per disegnare i marker sulla mappaS
