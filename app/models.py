#cript che definisce la strutturr del database, tabelle, colonne, relazioni tra dati.
#descrive come vengono salvati utenti e fermate nel file app.db
...
#tabella di associazione che collega utenti e fermate
#utilizzato per tenere traccia delle fermate salvate dall'utente
saved_locations = db.Table('saved_locations',
                           db.Column('user_id', db.Integer, db.ForeignKey('user.id'), primary_key=True),
                           db.Column('location_id', db.Integer, db.ForeignKey('location.id'), primary_key=True)
                           )

#Tabelle utenti nel db
...
#funzione richiesta da flask-login, chiamata quando l'utente fa una richiesta al server
#questa funzione viene utilizzata per caricare l'utente dal db  tra,ite il suo ID
@login.user_loader
def load_user(id):
